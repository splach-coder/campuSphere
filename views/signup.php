...
// If the session contains form data, display it
if (isset($_SESSION["form_data"])) {
    $formdata = $_SESSION["form_data"];
    unset($_SESSION["form_data"]); // Remove the form data from the session
}
?>
<html>
...
<head>
    ...
    <title>campuSphere: Sign up</title>
                ...
                <div class="formbg-outer">
                    <div class="formbg">
                        <div class="formbg-inner padding-horizontal--48">
                            ...
                            <form id="stripe-login" method="post" action="../controller/signupController.php" onsubmit="return validateForm()">
                                <?php if (isset($_GET['message']) && isset($_GET['type'])) { ?>
                                    <div class="alert alert-<?= $_GET['type'] ?>" role="alert">
                                        <?= $_GET['message'] ?>
                                    </div>
                                <?php } ?>
                                <div class="field padding-bottom--24" <label for="username">username</label>
                                    <input type="text" id="username" name="username" value="<?= (isset($formdata)) ? $formdata['username'] : '' ?>">
                                </div>
                                <div class="field padding-bottom--24">
                                    <label for="firstname">first name</label>
                                    <input type="text" id="firstname" name="firstname" value="<?= (isset($formdata)) ? $formdata['firstname'] : '' ?>">
                                </div>
                                <div class="field padding-bottom--24">
                                    <label for="lastname">last name</label>
                                    <input type="text" id="lastname" name="lastname" value="<?= (isset($formdata)) ? $formdata['lastname'] : '' ?>">
                                </div>
                                <div class="field padding-bottom--24">
                                    <label for="email">Email</label>
                                    <input type="email" id="email" name="email" value="<?= (isset($formdata)) ? $formdata['email'] : '' ?>">
                                </div>
                                <div class="field padding-bottom--24">
